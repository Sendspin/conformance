...
The aiosendspin SDK now requires every connection to be Noise-encrypted and
paired: each side needs a long-term ``Identity`` keypair and a pairing store
seeded with a long-term PSK before a "real" (non-sentinel) session can be
established.

Conformance cases run the server and client adapters as two independent
processes with no side-channel beyond the CLI args the harness already passes
(``server_id``/``client_id``). Rather than inventing a new IPC handshake to
exchange keys/PSKs out of band, both processes deterministically derive the
*same* identity and long-term PSK from those already-shared string ids. This
is intentionally not cryptographically meaningful (the "secret" is derivable
by anyone who knows the ids) — it only needs to make both sides agree on a
pre-paired long-term credential so the case can exercise the real Noise
handshake and role-activation path the spec requires.
"""
...
async def make_server_identity_and_store(*, server_id: str, client_id: str) -> tuple[Any, Any]:
    ...
    can use the real (non-sentinel) long-term PSK path.
    """
